...
def get_pagination_keyboard(items, page: int, query_id: str):
    ...
    for item in items[start:end]:
        ...
        builder.row(
            types.InlineKeyboardButton(
                text=f"🎵 {title}", callback_data=f"dl_{item['id']}"
            )
        )
    ...
    if page > 0:
        nav.append(
            types.InlineKeyboardButton(
                text="⬅️ Назад", callback_data=f"page_{query_id}_{page-1}"
            )
        )
    ...
    if end < len(items):
        nav.append(
            types.InlineKeyboardButton(
                text="➡️ Вперед", callback_data=f"page_{query_id}_{page+1}"
            )
        )
        ...
        with yt_dlp.YoutubeDL({"extract_flat": True, "quiet": True}) as ydl:
            ...
            info = await loop.run_in_executor(
                ...
                lambda: ydl.extract_info(
                    f"ytsearch10:{message.text}", download=False
                ),
            )
...
async def pages(callback: types.CallbackQuery):
    ...
    await callback.message.edit_reply_markup(
        reply_markup=get_pagination_keyboard(
            search_cache[query_id], page, query_id
        )
    )
...
async def download(callback: types.CallbackQuery):
    ...
    if video_id in music_cache and os.path.exists(music_cache[video_id]):
        await callback.message.answer_audio(
            audio=FSInputFile(music_cache[video_id])
        )
        return
